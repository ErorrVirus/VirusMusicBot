        ...
        if (token) {
            ...
            if (tracks.length > 0) {
                ...
                try {
                    ...
                    console.warn('[Spotify] Failed to get playlist name from fallback:', e.message);
                }
                return { name, tracks };
            }
        }
            ...
            while (Date.now() < deadline) {
                ...
                if (offset === 0) {
                    try {
                        ...
                        console.warn('[Spotify] Failed to get album details from API:', e.message);
                    }
                }
...
function toSearchQuery(track) {
    ...
    return `ytsearch:${track.name} ${artist}`.trim();
}
